...
const GUARDRAIL = resolve(dirname(fileURLToPath(import.meta.url)), '..', 'payload', 'guardrail-check.mjs');
...
test('derives its deny pattern from .linked-repos.json and denies a matching path with the gate closed', async () => {
  const root = await workbenchWithTargets({Shrimp: {path: 'Soniferous-Shrimp', remote: 'https://github.com/example/shrimp.git', branch: 'workbench/x', upstream: 'origin/main'}});
  const r = run(root, {hookEventName: 'PreToolUse', tool_name: 'Read', tool_input: {file_path: 'Soniferous-Shrimp/AGENTS.md'}});
  assert.equal(r.stdout.hookSpecificOutput.permissionDecision, 'deny', JSON.stringify(r));
  assert.match(r.stdout.hookSpecificOutput.permissionDecisionReason, /Soniferous-Shrimp/);
});
...
test('a second registered target is guarded too, named correctly in the deny reason', async () => {
  const root = await workbenchWithTargets({
    Shrimp: {path: 'Soniferous-Shrimp', remote: 'https://github.com/example/shrimp.git', branch: 'workbench/x', upstream: 'origin/main'},
...
test('the Claude Code contract (stderr + exit 2) still works for a payload without hookEventName', async () => {
  const root = await workbenchWithTargets({Shrimp: {path: 'Soniferous-Shrimp', remote: 'https://github.com/example/shrimp.git', branch: 'workbench/x', upstream: 'origin/main'}});
  const r = run(root, {tool_name: 'Read', tool_input: {file_path: 'Soniferous-Shrimp/AGENTS.md'}});
  assert.equal(r.status, 2, JSON.stringify(r));
  assert.match(r.stderr, /Soniferous-Shrimp/);
});
